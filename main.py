# ================= Witanime API - FastAPI + CloudScraper Version =================
import cloudscraper
import re
import base64
import urllib.parse
...
app = FastAPI(title="Witanime API", description="API لاستخراج بيانات الأنمي من witanime.you (CloudScraper)", version="1.1")

website = "https://witanime.you/"
HEADERS = {"User-Agent": "Mozilla/5.0"}

# إنشاء سكرابر واحد يعاد استخدامه (لتجنب إنشاء جلسات متعددة)
...
def root():
    return {
        "message": "مرحباً بك في Witanime API (CloudScraper)",
        "endpoints": {
...
def anime_details(url: str = Query(..., description="رابط صفحة الأنمي مثل: https://witanime.you/anime/...")):
    try:
        ...
        rss_resp = scraper.get(rss_url, headers=HEADERS)
